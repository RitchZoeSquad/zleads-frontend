...
import { setriskyLeads } from '@/redux/reducers/userSlice'
import ProtectedRoute from '@/components/PrivateRoute'
import LeadTable from '@/components/LeadsTable'
import { Stack } from '@/utils/StackForPagination'

function Page() {
  const { riskyLeads, isloggedin } = useSelector((state) => state.users);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/services/riskyleads?page=${currentPage}&typeOfSearch=${type}&type=${DateParameter}`,{
        lastEvaluatedKey  : lastEvaluatedKey
      });

      const { items, totalPages, lastCreatedAt } = response.data;

      dispatch(setriskyLeads(items));
        ...
        <div className={styles.home}>
          ...
          <div className={styles.headingP}>
                  ...
                  <button onClick={() => { model === true ? setShowModel(false) : setShowModel(true) }} className={styles.filterIcon}>
                    <Image  alt="icon" src="/icons/Frame.png" width={24} height={24} />
                  </button>
                </div>
          ...
          {loading === true ?
            ...
            riskyLeads?.length === 0 ?
              ...
              : <>
                ...
                <LeadTable url={riskyLeads[0].lead.file_url} risky={true} data={riskyLeads} type={type} />
